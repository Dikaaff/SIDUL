...
<div class="space-y-6">
    {{-- Main Container --}}
    <div class="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden font-sans">
        ...
        <div class="p-4 md:p-8">
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
                @forelse($users as $u)
                <div class="bg-white hover:bg-gray-50 border border-gray-100 rounded-[2rem] p-6 transition-all group shadow-sm hover:shadow-md flex flex-col justify-between">
                    <div class="flex items-start justify-between mb-6">
                        ...
                        @if($u->role === 'dosen')
                            <span class="px-3 py-1 rounded-lg bg-amber-50 text-amber-600 text-[8px] font-black uppercase tracking-[0.15em] border border-amber-100">Dosen</span>
                        @else
                            <span class="px-3 py-1 rounded-lg bg-blue-50 text-blue-600 text-[8px] font-black uppercase tracking-[0.15em] border border-blue-100">Operator</span>
                        @endif
                    </div>
                ...
                <div class="col-span-full py-24 text-center">
                    <div class="w-24 h-24 bg-gray-50 rounded-[2.5rem] flex items-center justify-center mx-auto mb-6 text-gray-200">
...
<!-- Modal Tambah User -->
<x-modal id="add_user_modal" title="Tambah Akun Baru" width="md">
    <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-6">
        ...
        <div class="space-y-5 font-sans">
            ...
            <div>
                ...
                <input type="text" name="name" class="input input-bordered w-full rounded-2xl bg-gray-50 border-gray-100 focus:border-primary/30 font-bold text-sm h-14" placeholder="Contoh: Dr. Budi Santoso, S.T., M.T." required>
            </div>
            
            <!-- Username / NIK -->
            <div>
                <label class="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2 block">Username / NIK</label>
                <input type="text" name="username" class="input input-bordered w-full rounded-2xl bg-gray-50 border-gray-100 focus:border-primary/30 font-bold text-sm h-14" placeholder="Masukkan NIK atau Username" required>
            </div>

            <!-- Password -->
            <div>
                <label class="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2 block">Password Baru</label>
                <input type="password" name="password" minlength="8" class="input input-bordered w-full rounded-2xl bg-gray-50 border-gray-100 focus:border-primary/30 font-bold text-sm h-14" placeholder="Minimal 8 karakter" required>
            </div>
