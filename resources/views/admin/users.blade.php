...
>
    <button onclick="openModalAddUserModal()" class="btn min-h-0 h-10 px-5 rounded-2xl bg-amber-400 hover:bg-amber-500 text-white border-none font-black text-xs uppercase tracking-widest">
...
<div class="space-y-6">
    {{-- Main Container --}}
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden font-sans">
        ...
        <div class="p-4 md:p-8">
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
                @forelse($users as $u)
                <div class="bg-white hover:bg-gray-50 border border-gray-100 rounded-2xl p-6 transition-all group shadow-sm hover:shadow-md flex flex-col justify-between">
                    <div class="flex items-start justify-between mb-6">
                        ...
                        @if($u->role === 'dosen')
                            <span class="px-3 py-1 rounded-2xl bg-amber-50 text-amber-600 text-[8px] font-black uppercase tracking-[0.15em] border border-amber-100">Dosen</span>
                        @else
                            <span class="px-3 py-1 rounded-2xl bg-blue-50 text-blue-600 text-[8px] font-black uppercase tracking-[0.15em] border border-blue-100">Operator</span>
                        @endif
                    </div>
                ...
                <div class="col-span-full py-24 text-center">
                    <div class="w-24 h-24 bg-gray-50 rounded-2xl flex items-center justify-center mx-auto mb-6 text-gray-200">
    ...
    <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-6">
        ...
        <div class="space-y-5 font-sans">
            ...
            <div class="flex flex-col gap-3 pt-6 border-t border-gray-50 mt-8">
                <button type="submit" class="btn h-14 rounded-2xl bg-amber-400 hover:bg-amber-500 border-none text-white font-black text-xs uppercase tracking-widest shadow-lg shadow-amber-100 active:scale-95 transition-all">
                    Simpan Akun Baru
                </button>
...
<x-modal id="delete_user_modal" title="Hapus Akun" subtitle="Tindakan ini tidak dapat dibatalkan." size="sm">
    <div class="space-y-6">
        ...
        <form id="deleteUserForm" method="POST" class="flex flex-col gap-3 pt-2">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn h-14 rounded-2xl bg-red-500 hover:bg-red-600 border-none text-white font-black text-xs uppercase tracking-widest shadow-lg shadow-red-500/20 active:scale-95 transition-all">
                Ya, Hapus Akun
            </button>
            <button type="button" onclick="closeModalDeleteUserModal()" class="btn h-14 rounded-2xl bg-gray-50 hover:bg-gray-100 border-none text-gray-500 font-black text-xs uppercase tracking-widest transition-all">
                Batalkan
            </button>
        </form>
    </div>
