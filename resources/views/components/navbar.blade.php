            ...
            <ul tabindex="0" class="mt-3 z-[1] p-3 shadow-xl menu menu-sm dropdown-content bg-base-100 rounded-box w-64 border border-base-200 gap-1 animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                <li class="mt-1 p-0">
                    <form action="/logout" method="POST" class="block w-full">
                        @csrf
                        <button type="submit" class="w-full h-full flex items-center gap-3 px-3 py-3 hover:bg-red-50 text-red-500 group transition-colors rounded">
                            <div class="w-8 h-8 rounded bg-red-50 flex items-center justify-center group-hover:bg-white shadow-sm transition-colors text-red-400">
