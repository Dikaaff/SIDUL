...
<div class="grid grid-cols-1 gap-8 font-sans">
    ...
    <x-card padding="none" border class="overflow-hidden">
        <div class="px-8 py-5 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
            ...
            <span class="px-3 py-1 bg-purple-50 text-[#6B21A8] text-[10px] font-black rounded border border-purple-100 uppercase tracking-widest">{{ $belumAssign->count() }} Mahasiswa</span>
        </div>
            ...
            <table class="table w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-all group">
                        ...
                        <td>
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded bg-purple-50 text-[#6B21A8] font-black flex items-center justify-center text-xs">
                        ...
                        <td>
                            <form id="assign-form-{{ $magang->id }}" action="{{ route('operator.assign_dosen', $magang->id) }}" method="POST">
                                @csrf
                                <select name="dosen_id" class="select select-sm select-bordered w-full max-w-xs rounded bg-white border-gray-200 text-gray-700 font-bold focus:border-[#6B21A8]" required>
                    ...
                    <tr>
                        <td colspan="5" class="py-24 text-center">
                            <div class="flex flex-col items-center gap-4">
                                <div class="w-20 h-20 rounded bg-gray-50 flex items-center justify-center text-gray-300 mb-2 border-2 border-dashed border-gray-100">
                ...
                <tbody class="divide-y divide-gray-100 text-sm">
                    ...
                    <tr class="hover:bg-gray-50 transition-all">
                        ...
                        <td class="pr-8 text-right flex items-center justify-end gap-2">
                             <span class="px-3 py-1.5 rounded bg-green-50 text-green-600 text-[9px] font-black uppercase tracking-wider border border-green-100">Plotting Aktif</span>
                             <form action="{{ route('operator.magang.destroy', $magang->id) }}" method="POST" onsubmit="return confirm('Hapus data magang ini?')">
                                 @csrf
                                 @method('DELETE')
                                 <button type="submit" aria-label="Hapus plotting" class="btn btn-ghost btn-xs text-red-500 hover:bg-red-50 rounded p-1">
